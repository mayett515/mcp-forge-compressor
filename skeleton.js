...
function getCallExpressionName(node) {
  for (const child of node.children) {
    if (child.type === "variable_declarator") {
      ...
      // FIX: Arrays and objects are never hook calls — bail immediately to prevent
      // false-positives on data structures that merely *contain* hook names as strings.
      if (
        !value ||
        value.type === "array" ||
...
function getExpressionCallName(node) {
  const expr = node.children.find(
    (c) => c.type === "call_expression" || c.type === "await_expression",
  );
  if (!expr) return null;
  const call =
    expr.type === "await_expression"
...
const FRAMEWORK_PATTERNS = [
  ...
  // Vue Reactivity: const x = ref(...) / reactive(...) / computed(...)
  {
    test(node) {
      ...
      const callName = getCallExpressionName(node);
      return (
        callName === "ref" || callName === "reactive" || callName === "computed"
      );
    },
    label(node) {
      const decl = node.text.replace(/\s+/g, " ").replace(/;$/, "").trim();
  ...
  // Express/Nest Routing: app.get('/path', handler)
  {
    test(node) {
      ...
      return /^(app|router)\.(get|post|put|patch|delete|use|all|options|head)$/.test(
        callName,
      );
    },
    label(node) {
      const match = node.text.match(
        /\b(app|router)\.(get|post|put|patch|delete|use|all|options|head)\s*\(\s*['"`]([^'"`]*)['"`]/,
      );
      if (match) {
        return `Route: ${match[1]}.${match[2]}('${match[3]}')`;
      }
      const methodMatch = node.text.match(
        /\b(app|router)\.(get|post|put|patch|delete|use|all|options|head)/,
      );
      if (methodMatch) {
    ...
    const hasInnerContent =
      ...
      body.children.some((c) => {
        if (c.type === "function_declaration" || c.type === "class_declaration")
          return true;
        if (matchFrameworkPattern(c) !== null) return true;
        // Only count lexical_declaration if it contains an arrow/function expression
        if (
